...
export default class Mastodon {
   ...
   */
  private getBaseUrl = () => `${this.url}${this.urlVersion}`;

  /**
   * Getting base url of streaming API
   * @return Base url of streaming API
   */
  private getStreamingBaseUrl = () => `${this.getStreamingUrl()}${this.getUrlVersion()}`;
   ...
   */
  public fetchFavouritedStatuses = (options?: Mastodon.FetchFavouritedStatuses): Promise<Mastodon.Status[]> => {
   ...
   */
  public fetchNotifications = (options?: Mastodon.FetchNotifications): Promise<Mastodon.Notification[]> => {
